...
module.exports.bootstrap = async function(done) {
  ...
  for (var idx = 0; idx < statusid.length; idx++) {
    var status = await ServiceStatus.find({statusID: statusid[idx]});

    if (status.length == 0) {
      console.log("Creating status %s", statusname[idx]);
      await ServiceStatus.create({ statusName: statusname[idx], statusID: statusid[idx] });
    }
  }
